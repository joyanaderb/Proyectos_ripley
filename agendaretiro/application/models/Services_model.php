...
class Services_model extends EA_Model {
     ...
     public function getagendamientos()
    {
        ...
        $this->db->select("*, t1.first_name nombre, t2.first_name as sucursal, t2.last_name as comuna, t1.IdCliente as rut, t1.OC as oc, t1.Motivo2 as motivo2, t1.last_name as ticketauris, t1.address as direccion, t1.phone_number as telefono, t1.email as correo, t1.zip_code as CUD, t1.notes as notes");
         $this->db->join('ea_users as t1', 'ea_appointments.id_users_customer = t1.id');
           $this->db->join('ea_users as t2', 'ea_appointments.id_users_provider = t2.id');
        $resultados = $this->db->get("ea_appointments");
       ...
       public function get_setting($service)
    {
           
           
           
       return $this->db->get_where('services', ['id' => (int)$service])->row()->book_advance_timeout;
     ...
     public function motivoReclamo()
    {
        ...
        $NumCampana = $this->input->GET('NumCampana', TRUE);
          $data = $db7->query("SELECT mot_desmot2 from motivoscasos where mot_nummot1 = '$Motivo1' and mot_nummot2 = '$Motivo2' and mot_numcam = '$NumCampana' limit 1");
if ($data->num_rows() > 0) {
            return $data->result();
        } else {
            return false; 
