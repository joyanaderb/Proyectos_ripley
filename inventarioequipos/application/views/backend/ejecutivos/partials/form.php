...
<div class="col-md-6">
    ...
    <div class="form-group <?php echo form_error('dni') == true ? 'has-error' : '' ?>">
        <label for="dni">RUT ejemplo:(18979626-9)</label>
        <?php if (isset($lector->dni)): ?>
            <?php $valor = form_error('dni') == true ? set_value('dni') : $lector->dni;?>;
            <input type="text" class="form-control" id="dni" name="dni" placeholder="RUT" value="<?php echo $valor; ?>">
        <?php else: ?>
            <input type="text" class="form-control" id="dni" name="dni" placeholder="RUT" value="<?php echo set_value('dni'); ?>">
        <?php endif;?>
...
<div class="col-md-6">
	...
	<div class="form-group <?php echo form_error('id') == true ? 'has-error' : '' ?>">
        <label for="id" class="control-label">Tipo de Personal</label>
        <select name="id" id="id" class="form-control">
            <option value="">Seleccione...</option>
            <?php foreach ($estados as $estados_agentes): ?>
                <?php if (isset($lector->estado) && $estados_agentes->id == $lector->id): ?>
                    <option value="<?php echo $estados_agentes->id; ?>" selected="selected"><?php echo $estado_agentes->estado_agente; ?></option>
                <?php else: ?>
                    <option value="<?php echo $estados_agentes->id; ?>"><?php echo $estados_agentes->estado_agente; ?></option>
                <?php endif;?>
            <?php endforeach;?>
        </select>
    ...
    <div class="form-group <?php echo form_error('idtipolector') == true ? 'has-error' : '' ?>">
        <label for="idtipolector" class="control-label">Tipo de Personal</label>
